...
const PinInfoPopup: React.FC<PinInfoPopupProps> = ({
  ...
  return (
    <div className="w-[340px] max-w-[92vw] rounded-2xl bg-white/95 shadow-xl backdrop-blur-md border border-gray-200 flex flex-col animate-fade-in max-h-[90vh] overflow-y-auto">
      <div className="h-14 w-full relative rounded-t-2xl flex items-center justify-between px-5 py-3 bg-gradient-to-r from-white/80 via-gray-50/80 to-white/60 border-b border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900 truncate flex-1 pr-3">
          {pin.name || 'Unnamed Memory'}
        </h3>
        {onClose && (
          <button
            className="cursor-pointer bg-gray-200/70 hover:bg-gray-300 transition-colors rounded-full w-8 h-8 flex items-center justify-center shadow focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-400 flex-shrink-0"
            onClick={onClose}
            aria-label="Close"
            title="Close"
          >
      ...
      <div className="px-5 py-3 flex flex-col gap-2">
        <button
          type="button"
          className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-50 text-indigo-700 font-medium py-2 px-3 hover:bg-indigo-100 transition border border-indigo-200 cursor-pointer"
          onClick={() => {
            if (pin.isOwner) {
              onViewProfile(null);
            } else {
      ...
      {showTimestamp && pin.timestamp && (
        <div className="px-5 py-2 text-right">
          <span className="timestamp text-sm font-medium text-gray-700">
